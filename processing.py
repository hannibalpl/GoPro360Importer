...
def get_video_metadata(video_path, log_widget=None):
    ...
    proc = subprocess.run(fps_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, creationflags=CREATE_NO_WINDOW)
    if proc.returncode != 0:
    ...
    dur_cmd = [FFPROBE_BIN, '-v', 'error', '-show_entries', 'format=duration',
               '-of', 'default=noprint_wrappers=1:nokey=1', video_path]
    proc = subprocess.run(dur_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, creationflags=CREATE_NO_WINDOW)
    if proc.returncode != 0:
...
def parse_csv_telemetry(csv_path, log_widget=None):
    ...
    with open(csv_path, newline='', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            coords.append({'timestamp': dt.timestamp(), 'datetime': dt, 'lat': float(row['GPS (Lat.) [deg]']), 'lon': float(row['GPS (Long.) [deg]'])})
